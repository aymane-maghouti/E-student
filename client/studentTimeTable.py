...
class StudentTimeTable:
    ...
    def createStudentTimeTable(self, base):
        ...
        Label(table, text="Class", width=5, height=2,font=("Montserrat", 12, "bold"),foreground="#bb86fc",background="#1f1a24").grid(row=0, column=0)
        Label(table, text="Download", width=15, height=2,font=("Montserrat", 12, "bold"),foreground="#bb86fc",background="#1f1a24").grid(row=0, column=1)
        ...
        i=0
        for row in result:
            Label(table, text=row[0], width=10, height=2,font=("Montserrat", 10),foreground="white",background="#1f1a24").grid(row=i + 1, column=0)
            downloadButton=tk.Button(table,relief="sunken",borderwidth=0,cursor="hand2",activebackground="#1f1a24",background="#1f1a24",image=self.downloadStandardIcon,command=lambda index=i:download_button_click(index))
            downloadButton.grid(row=i+1, column=1)
            Label(table, text=row[2], width=20, height=2,font=("Montserrat", 10),foreground="white",background="#1f1a24").grid(row=i + 1, column=2)
            i+=1
