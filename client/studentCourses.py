...
class StudentCourses:
    ...
    def createStudentCourses(self, base):
        ...
        base.studentCoursesFrame = MyScrollableFrame(base.Background, base.studentCoursesImg, "#1f1a24", 685, 297, 117,
                                                     174, 20, 20)
    ...
    def documents(self):
        ...
        Label(self.table, text="Title", width=30, height=2, font=("Montserrat", 12, "bold"), foreground="#bb86fc",
              background="#1f1a24").grid(row=0, column=1)
        Label(self.table, text="Download", width=10, height=2, font=("Montserrat", 12, "bold"), foreground="#bb86fc",
              background="#1f1a24").grid(row=0, column=2)
        Label(self.table, text="Published at", width=15, height=2, font=("Montserrat", 12, "bold"),
        ...
        i = 0
        for row in result:
            Label(self.table, text=row[0], width=5, height=2, font=("Montserrat", 10), foreground="white",
                  background="#1f1a24", ).grid(row=i + 1, column=0)
            Label(self.table, text=row[1], width=40, height=2, font=("Montserrat", 10), foreground="white",
                  background="#1f1a24", ).grid(row=i + 1, column=1)
            downloadButton = tk.Button(self.table, relief="sunken", borderwidth=0, cursor="hand2",
                                       activebackground="#1f1a24", background="#1f1a24",
                                       image=self.downloadStandardIcon,
                                       command=lambda index=i: self.download_button_click(index))
            downloadButton.grid(row=i + 1, column=2)
            Label(self.table, text=row[3], width=17, height=2, font=("Montserrat", 10), foreground="white",
