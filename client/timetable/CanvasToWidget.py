...
class MyButton():
    def __init__(self, base, x, y, standardImg, cursor="arrow", hoverImg=None, clickImg=None, behavior=lambda: None,
                 entry=None, padx=0, pady=0, fgSelected="black", fgNotSelected="white", *args, **kwargs):
        self.x = x
        self.y = y
        self.padx = padx
        self.pady = pady
        ...
        if self.entry != None:
            self.entry = self.base.create_text(self.x + self.padx, self.y + self.pady, text=entry,
                                               fill=self.fgNotSelected, *args, **kwargs)
...
class MyEntry:
    def __init__(self, base, x, y, standardImg, entry=None, hoverImg=None, clickImg=None, behavior=lambda: None,
                 marginX=0, marginY=0, placeholder="", modified=False, value=None):
        self.x = x
        self.y = y
...
class MyMenu:
    def __init__(self, base, x, y, textLabel, defaultMenuButtonImg, hoverMenuButtonImg, clickedMenuButtonImg,
                 defaultMenuListImg, hideWidgets=[],
                 padx=10, pady=7, menuListMarginX=0, menuListMarginY=40, listBoxMarginX=10, listBoxMarginY=50,
                 anchor=NW,
                 standardbg="#1f1a24", standardfg="white", highlightbg="#bb86fc", highlightfg="#1f1a24", options=[],
                 *args, **kwargs):
        ...
        self.menuList = MyListBox(self.base, standardbg=self.standardbg, standardfg=self.standardfg,
                                  highlightbg=self.highlightbg, highlightfg=self.highlightfg, options=options, *args,
                                  **kwargs)
    ...
    def menuButtonClick(self, event):
        if self.isActive == False:
            ...
            self.clickedMenuListObject = self.base.create_image(self.x + self.menuListMarginX,
                                                                self.y + self.menuListMarginY,
                                                                image=self.defaultMenuListPhoto, anchor=NW)

            # saving the coorinates of widgets to hide while menu is active
            for widget in self.hideWidgets:
                print(widget.place_info()["x"])
                self.hiddenWidgetsPlaces.append(
                    (widget, (int(widget.place_info()["x"]), int(widget.place_info()["y"]))))
...
class MyOption(MyButton):
    def __init__(self, base, x, y, entry=None, value=None, fgSelected="blue", fgNotSelected="blue", text="option",
                 *args, **kwargs):
...
class MyFrame:
    def __init__(self, baseCanvas, backgroundImg, backgroundColor, width, height, x=0, y=0, padx=0, pady=0):
        ...
        self.mainFrame = Canvas(baseCanvas, border=0, highlightthickness=0, background=backgroundColor, width=width,
                                height=height)
