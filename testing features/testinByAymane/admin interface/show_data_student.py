...
def id1_students_page():
    ...
    cursor.execute("SELECT name,age,image FROM students where filier='ID' ")
    ...
    id1_students_frame.pack()

    # Column headers row 0
    l1 = tk.Label(id1_students_frame, text='Nom complet')
    l1.grid(row=0, column=1)

    l2 = tk.Label(id1_students_frame, text='Âge')
    l2.grid(row=0, column=2)

    l3 = tk.Label(id1_students_frame, text='Photo')
    ...
    for student in my_row:
        ...
        e = tk.Label(id1_students_frame, text=student[0])
        e.grid(row=i, column=1, ipadx=20)
        e = tk.Label(id1_students_frame, text=student[1])
        e.grid(row=i, column=2, ipadx=60)
        e = tk.Label(id1_students_frame, image=img)
...
def gc1_students_page():
    ...
    cursor.execute("SELECT name,age,image FROM students where filier='GC' ")
    my_row = cursor.fetchall()

    # create Tkinter window and frame

    gc1_students_frame = tk.Frame(main_frame)
    gc1_students_frame.pack()

    # Column headers row 0
    l1 = tk.Label(gc1_students_frame, text='Nom complet')
    l1.grid(row=0, column=1)

    l2 = tk.Label(gc1_students_frame, text='Âge')
    l2.grid(row=0, column=2)

    l3 = tk.Label(gc1_students_frame, text='Photo')
    l3.grid(row=0, column=3)
    ...
    for student in my_row:
        ...
        img = Image.open(stream)
        img = ImageTk.PhotoImage(img)
        e = tk.Label(gc1_students_frame, text=student[0])
        e.grid(row=i, column=1, ipadx=20)
        e = tk.Label(gc1_students_frame, text=student[1])
        e.grid(row=i, column=2, ipadx=60)
        e = tk.Label(gc1_students_frame, image=img)
        e.grid(row=i, column=3, ipady=7)
        images.append(img)  # garbage collection
        i = i + 1
...
def gi1_students_page():
    ...
    cursor.execute("SELECT name,age,image FROM students where filier='GI' ")
    my_row = cursor.fetchall()

    # create Tkinter window and frame

    gi1_students_frame = tk.Frame(main_frame)
    gi1_students_frame.pack()

    # Column headers row 0
    l1 = tk.Label(gi1_students_frame, text='Nom complet')
    l1.grid(row=0, column=1)

    l2 = tk.Label(gi1_students_frame, text='Âge')
    l2.grid(row=0, column=2)

    l3 = tk.Label(gi1_students_frame, text='Photo')
    l3.grid(row=0, column=3)
    ...
    for student in my_row:
        ...
        img = Image.open(stream)
        img = ImageTk.PhotoImage(img)
        e = tk.Label(gi1_students_frame, text=student[0])
        e.grid(row=i, column=1, ipadx=20)
        e = tk.Label(gi1_students_frame, text=student[1])
        e.grid(row=i, column=2, ipadx=60)
        e = tk.Label(gi1_students_frame, image=img)
        e.grid(row=i, column=3, ipady=7)
        images.append(img)  # garbage collection
        i = i + 1
