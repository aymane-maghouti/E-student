...
# TABLE : filiere
mycursor.execute(
    "create table if not exists filiere(id_filiere int primary key auto_increment,name varchar(45),description varchar(200),id_departement int not null,foreign key (id_departement) references departement(id_departement))")

# TABLE : class
mycursor.execute(
    "create table if not exists class(id_class int primary key auto_increment,name_class varchar(45),id_filiere int not null,foreign key (id_filiere) references filiere(id_filiere))")
...
# TABLE : bac_student
mycursor.execute(
    "create table if not exists bac_student(id_bac_student int primary key auto_increment,id_student int not null,bac_filiere varchar(45),bac_language varchar(45),grid numeric(4,2),bac_city varchar(45),school_city varchar(45),school_type varchar(45),foreign key (id_student) references student(id_student))")
# TABLE : contact
mycursor.execute(
...
# TABLE : login
mycursor.execute(
    "create table if not exists login (id_login int primary key auto_increment,id_filiere int not null,email_acadymic varchar(45) ,password varchar(200),foreign key (id_filiere) references filiere(id_filiere))")
# TABLE : notification
mycursor.execute(
    "create table if not exists Notification (id_notification int primary key auto_increment,id_filiere int not null,title varchar(200),detail varchar(9999),date_not datetime,foreign key (id_filiere) references filiere(id_filiere))")
# TABLE : admin
mycursor.execute(
...
# TABLE : filiere_student
mycursor.execute(
    "CREATE table if not exists filiere_student (id_filiere_student int primary key auto_increment,id_filiere int not null,id_student int not null,foreign key (id_filiere) references filiere(id_filiere),foreign key (id_student) references student(id_student))")
# TABLE : Affichage
mycursor.execute(
